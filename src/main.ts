...
//!Все события наследуются от Event target
//?События мыши:
...
//!неправильно
//button.onclick = sayThanks();
//Если добавить скобки, то sayThanks() – это уже вызов функции, результат которого
//(равный undefined, так как функция ничего не возвращает) будет присвоен onclick. Так что это не будет работать.
// в разметке, в отличие от свойства, скобки нужны:
...
//?методы addEventListener -Метод addEventListener позволяет добавлять несколько обработчиков на одно событие одного элемента
...
//event.clientX / event.clientY - координаты курсора в момент клика относительно окна, для событий мышь.

//Объект-обработчик: handleEvent метод может вызывать другие методы ПОЧИТАТЬ!!!
...
//Событие mousemove возникает хоть и часто, но не для каждого пикселя. Поэтому из-за быстрого движения указатель может спрыгнуть.
//Мы должны отслеживать mousemove на всём document, чтобы поймать его.

//Существует метод document.elementFromPoint(clientX, clientY). Он возвращает наиболее глубоко вложенный элемент по заданным
//координатам окна (или null, если указанные координаты находятся за пределами окна).
...
// let currentTD = null as HTMLTableCellElement | null | string
...
//   td.classList.add('edit-td')//редактирование
...
//   //insertAdjacentHTML позволяет вставитьстроку HTML кода в любое место страницы.
//   // @ts-ignore
//   td.insertAdjacentHTML("beforeEnd",
//     '<div class="edit-controls"><button class="edit-ok">OK</button><button class="edit-cancel">CANCEL</button></div>'
//   )  
// })
// function applyBtn(td, OK){
//   if(!OK)
//     return
